...
const BlogItem = ({ data, index }: Props) => {
  ...
  if (mdUp && latestPost) {
    return (
      <Card>
        ...
        <BlogCard title={title} index={index} id={id} createdDate={createdDate} />
  ...
  return (
    <Card>
      ...
      <BlogCard title={title} id={id} createdDate={createdDate} />
    </Card>
  );
};
...
export const BlogCard = ({ title, index, id, createdDate }: PostContentProps) => {
  const mdUp = useResponsive('up', 'md');
