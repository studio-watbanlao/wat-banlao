...
export const DEFAULT_TEMPLE_NAVIGATION: TempleNavigationItem[] = [
  ...
  {
    itemKey: 'temple-architecture',
    title: 'สถาปัตย์และสิ่งสำคัญ',
    path: '/banlao/architecture',
    parentKey: 'temple',
    sortOrder: 20,
    enabled: true,
    deepMatch: true,
  ...
  {
    itemKey: 'master-pramuan',
    title: 'หลวงปู่ประมวล ญาณวโร',
    path: '/parents/luang-pu-pramuan',
    parentKey: 'masters',
    sortOrder: 20,
    enabled: true,
    deepMatch: false,
  ...
  {
    itemKey: 'community-history',
    title: 'ประวัติชุมชน',
    path: '/community/community-history',
    parentKey: 'community',
    sortOrder: 10,
    enabled: true,
    deepMatch: false,
  ...
  {
    itemKey: 'community-leaders',
    title: 'ผู้นำชุมชน',
    path: '/community/community-leaders',
    parentKey: 'community',
    sortOrder: 20,
    enabled: true,
    deepMatch: false,
  ...
  {
    itemKey: 'community-school',
    title: 'โรงเรียนชุมชน',
    path: '/community/school',
    parentKey: 'community',
    sortOrder: 30,
    enabled: true,
    deepMatch: false,
...
const PUBLIC_ROUTE_MODULES: Array<[prefix: string, module: TempleModule]> = [
  ['/activity', 'activities'],
  ['/banlao/architecture', 'architectures'],
  ['/banlao/abbot', 'directory'],
  ['/banlao/monks', 'directory'],
  ['/fastival', 'festivals'],
  ['/article/blog', 'blogs'],
  ['/article/dharma', 'dharmas'],
  ['/parents/sacred', 'sacred'],
  ['/community', 'community_leaders'],
  ['/banlao/community-', 'community_leaders'],
  ['/banlao/school', 'community_leaders'],
];

export const getPublicRouteModule = (pathname: string) =>
  PUBLIC_ROUTE_MODULES.find(
    ([prefix]) =>
      pathname === prefix ||
...
const BANLAO_ONLY_ROUTES = new Set([
  '/banlao/community-history',
  '/banlao/school',
  '/community/community-history',
  '/community/school',
]);

export const isBanlaoOnlyPublicRoute = (pathname: string) =>
  BANLAO_ONLY_ROUTES.has(pathname.replace(/\/+$/, '') || '/');
...
export const filterTempleNavigation = (
  ...
  return moduleEnabledItems.filter((item) => !item.parentKey || enabledParents.has(item.parentKey));
};
