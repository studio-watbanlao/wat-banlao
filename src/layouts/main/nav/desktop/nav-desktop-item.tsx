...
const ItemRoot = styled(ButtonBase, { shouldForwardProp })<StyledState>(({
  ...
  const dotStyles: CSSObject = {
    ...
    ...(active && {
      ...dotTransitions.out,
      backgroundColor: 'var(--header-accent-color, currentColor)',
    }),
  };
  ...
  const rootItemStyles: CSSObject = {
    ...
    '&:hover': {
      color: 'var(--header-accent-color, currentColor)',
...
const ItemTitle = styled('span', { shouldForwardProp })<StyledState>(({ theme }) => ({
  ...
  color: 'inherit',
  variants: [
    { props: { variant: 'subItem' }, style: { fontSize: theme.typography.pxToRem(16) } },
